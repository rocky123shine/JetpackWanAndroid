...
import com.rocky.base.constants.NET_RESPONSE_CODE_ERROR
import com.rocky.base.net.NetResult
import com.rocky.base.serializer.AnyKSerializer
import com.rocky.base.serializer.CookieKSerializer
...
/** 默认 [Json] 解析
@JvmField public val encodeDefaults: Boolean = false, // 是否编码默认值
@JvmField public val ignoreUnknownKeys: Boolean = false,// 忽略未知 key 不抛出异常
@JvmField public val isLenient: Boolean = false, // 是否使用宽松模式
@JvmField public val allowStructuredMapKeys: Boolean = false,    // 是否允许将 key-value 转换为 数组
@JvmField public val prettyPrint: Boolean = false, // 是否对打印的 json 格式化
@JvmField public val prettyPrintIndent: String = "    ",// 指定打印缩进字符串
@JvmField public val coerceInputValues: Boolean = false,// 非空类型为空或找不到对应枚举时使用默认值
@JvmField public val useArrayPolymorphism: Boolean = false, // 将多态序列化为默认数组格式
@JvmField public val classDiscriminator: String = "type",// 多态序列化的类描述符属性的名称
@JvmField public val allowSpecialFloatingPointValues: Boolean = false,// 是否取消对特殊浮点值的规范
@JvmField public val serializersModule: SerializersModule = EmptySerializersModule // 指定序列化模块
 * */
